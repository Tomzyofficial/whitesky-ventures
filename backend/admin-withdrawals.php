  ...
  <main class="mt-[100px] text-sm">
    <div class="table_contents">
      <table class="table table-auto border-separate border border-slate-500 mx-auto w-[90%] overflow-auto">
        <caption class="pb-5">ALL CRYPTO WITHDRAWALS</caption>
...
$sql = "SELECT * FROM withdrawals LIMIT $start_from, $records_per_page";
...
if ($result->num_rows > 0) {
        ...
        ?>
                <tbody> 
                  <tr>
                    ...
                    <td class="border border-slate-500"><?= htmlentities($row['plan']); ?></td>
                    <td class="border border-slate-500"><?= htmlentities($row['crypto']); ?></td>
                    <td class="border border-slate-500">
                      <?= '$' . htmlentities($row['amount']); ?>
                    </td>
                    <td class="border border-slate-500">
                      <?= '$' . htmlentities($row['wallet']); ?>
                    </td>
                    <td class="border border-slate-500">
    ...
    // pagination links
    $sql = "SELECT COUNT(id) AS total FROM withdrawals";
    $result = $conn->query($sql);
